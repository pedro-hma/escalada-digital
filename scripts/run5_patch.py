...
p = Path('src/App.tsx')
s = p.read_text()
s = s.replace("import { Html } from '@react-three/drei';", "import { Html, OrbitControls } from '@react-three/drei';")
start = s.index('function Character3D(')
end = s.index('\nconst WORLD = {', start)
replacement = r'''function Character3D({ avatar, state = 'IDLE', scale = 1 }: { avatar: Avatar; state?: MoveState; scale?: number }) {
...
cs = css.read_text()
if '.preview-hint' not in cs:
    cs += '\n.preview3d{position:relative}.preview-hint{position:absolute;left:50%;bottom:10px;transform:translateX(-50%);z-index:3;padding:6px 10px;border-radius:999px;background:rgba(3,12,8,.72);border:1px solid rgba(114,244,169,.18);color:#91a79b;font-size:.7rem;white-space:nowrap;pointer-events:none}.preview3d canvas{cursor:grab}.preview3d canvas:active{cursor:grabbing}\n'
css.write_text(cs)
